...
FASTQ_R1_URL="https://ftp.ncbi.nlm.nih.gov/geo/samples/GSM2527nnn/GSM2527019/suppl/GSM2527019_siAGO2_2_nearbait_norm_counts.bedGraph.gz"

# Creating directories
mkdir -p "${WORKDIR}" "${REF_GENOME_DIR}" "${FASTQ_DIR}" "${TRIMMED_DIR}" "${RESULTS_DIR}" "${QC_DIR}"
cd "${WORKDIR}"

# Step 1: Setup and Data Dow.nload
...
LEADING:3 TRAILING:3 SLIDINGWINDOW:4:15 MINLEN:36
 ...
 exit 1 # Exit code diffetent form zero means there is an eror
